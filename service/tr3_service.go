...
func (s *tr3Service) ExportPembayaranRenewal(data request.RangeTanggalRequest) error {
	...
	headers := []string{"NO", "NAMA MEMBER", "TANGGAL BAYAR", "CARD", "JENIS BAYAR", "KD USER", "CONFIRMER", "KURIR", "NO TANDA TERIMA", "NO MESIN", "ASURANSI", "ASURANSI MOTOR", "HARGA POKOK", "DPP", "PPN", "DPP 11/12"}
	...
	for i, record := range dataPembayaran {
		...
		xlsx.SetCellValue(sheetName, fmt.Sprintf("J%d", i+3), record.NoMsn)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("K%d", i+3), record.MstCard.Asuransi)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("L%d", i+3), record.MstCard.AsuransiMotor)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("M%d", i+3), record.MstCard.HargaPokok)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("N%d", i+3), math.Round(dpp))
		xlsx.SetCellValue(sheetName, fmt.Sprintf("O%d", i+3), math.Round(dpp*0.11))
		xlsx.SetCellValue(sheetName, fmt.Sprintf("P%d", i+3), math.Round(dpp1112))
		startRow += 1
	}

	xlsx.MergeCell(sheetName, "A1", "N1")
	xlsx.SetCellStyle(sheetName, "A1", "P1", headerStyle)
	xlsx.SetCellStyle(sheetName, "A2", "P2", headerStyle)
	xlsx.SetCellStyle(sheetName, fmt.Sprintf("A%d", 3), fmt.Sprintf("P%d", startRow-1), borderStyle)
...
func (s *tr3Service) ExportDataRenewal(data request.DataRenewalRequest) (entity.DataRenewal, error) {
	...
	headers := []string{"KD DLR", "NAMA DEALER", "NO RANGKA", "NO MESIN", "NO KARTU", "NAMA STNK", "NAMA KARTU", "NAMA TERTANGGUNG", "JNS KARTU", "TGL MOHON", "TGL AWAL", "TGL AKHIR", "ALAMAT"}
	for _, sheet := range sheets {
		for i, header := range headers {
...
func writeDataRenewalToSheet(xlsx *excelize.File, sheetName string, data []entity.DataRenewal, headers []string) error {
	...
	for i, record := range data {
		...
		xlsx.SetCellValue(sheetName, fmt.Sprintf("A%d", i+2), kdDlrValue)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("B%d", i+2), record.NmDlr)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("C%d", i+2), NoRgkValue)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("D%d", i+2), record.NoMsn)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("E%d", i+2), record.NoKartu)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("F%d", i+2), record.NmCustomer)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("G%d", i+2), namaKartu)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("H%d", i+2), namaTertanggung)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("I%d", i+2), record.JnsCard)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("J%d", i+2), tglMohonValue)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("K%d", i+2), record.TglAwal.Format("2006-01-02"))
		xlsx.SetCellValue(sheetName, fmt.Sprintf("L%d", i+2), record.TglAkhir.Format("2006-01-02"))
		xlsx.SetCellValue(sheetName, fmt.Sprintf("M%d", i+2), formattedAddress)
	}
