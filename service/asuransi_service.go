...
type AsuransiService interface {
	...
	ExportReportTs(u string, tgl1 string, tgl2 string)
...
func (s *asuransiService) ExportReport(tgl1 string, tgl2 string) {
	...
	xlsx.SetCellStyle(rekapSheet, "A3", "F3", headerStyle)
	xlsx.SetColWidth(rekapSheet, "A", "F", 17)
	xlsx.MergeCell(rekapSheet, "A1", "G1")
	xlsx.SetCellValue(rekapSheet, "A1", "Report Asuransi Periode "+tgl1+" - "+tgl2)
	xlsx.SetCellStyle(rekapSheet, "A1", "G1", headerStyle)
	...
	xlsx.SetCellStyle(rekapSheet, fmt.Sprintf("A%d", awalBorder), fmt.Sprintf("F%d", startRow-1), borderStyle)
	...
	xlsx.SetCellStyle(rekapSheet, fmt.Sprintf("A%d", startRow), fmt.Sprintf("H%d", startRow), headerStyle)
	...
	xlsx.SetCellStyle(rekapSheet, fmt.Sprintf("A%d", awalBorder), fmt.Sprintf("H%d", startRow-1), borderStyle)
	...
	xlsx.SetCellStyle(pendingSheet, "A3", fmt.Sprintf("%s3", listCol[startCol-1]), headerStyle)
	...
	xlsx.SetCellStyle(pendingSheet, "A4", fmt.Sprintf("%s%d", listCol[startCol-1], startRow-1), borderStyle)
	xlsx.SetColWidth(pendingSheet, "A", listCol[startCol-1], 17)
	...
	xlsx.SetCellStyle(pendingSheet, fmt.Sprintf("A%d", startRow), fmt.Sprintf("%s%d", listCol[startCol-1], startRow), headerStyle)
	xlsx.SetColWidth(pendingSheet, "A", listCol[startCol-1], 17)
	...
	xlsx.SetCellStyle(pendingSheet, fmt.Sprintf("A%d", awalBorder), fmt.Sprintf("%s%d", listCol[startCol-1], startRow-1), borderStyle)
	xlsx.SetColWidth(pendingSheet, "A", listCol[startCol-1], 17)
	...
	xlsx.SetCellStyle(tdkBerminatSheet, "A3", fmt.Sprintf("%s3", listCol[startCol-1]), headerStyle)
	xlsx.SetColWidth(tdkBerminatSheet, "A", listCol[startCol-1], 17)
	...
	xlsx.SetCellStyle(tdkBerminatSheet, fmt.Sprintf("A%d", awalBorder), fmt.Sprintf("%s%d", listCol[startCol-1], startRow-1), borderStyle)
	...
	xlsx.SetCellStyle(tdkBerminatSheet, fmt.Sprintf("A%d", startRow), fmt.Sprintf("%s%d", listCol[startCol-1], startRow), headerStyle)
	xlsx.SetColWidth(tdkBerminatSheet, "A", listCol[startCol-1], 17)
...
func (s *asuransiService) ExportReportTs(u string, tgl1 string, tgl2 string) {
	xlsx := excelize.NewFile()
	...
	bulan := []string{"", "Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"}
	rekapSourceInfo := s.trR.RekapByStatusTeleBulan(u, tgl1, tgl2)
	rekapKdUser := s.trR.RekapTeleReport(u, tgl1, tgl2)
	user := s.uR.FindByUsername(u)

	startRow := 5
	rekapSheet := "Rekap"
	xlsx.SetSheetName(xlsx.GetSheetName(1), rekapSheet)
	xlsx.SetColWidth(rekapSheet, "A", "G", 14)
	xlsx.SetCellValue(rekapSheet, "A4", "Id User")
	xlsx.SetCellValue(rekapSheet, "B4", "Data Source")
	xlsx.SetCellValue(rekapSheet, "C4", "Bulan")
	xlsx.SetCellValue(rekapSheet, "D4", "Pending")
	xlsx.SetCellValue(rekapSheet, "E4", "Tidak Berminat")
	xlsx.SetCellValue(rekapSheet, "F4", "Berminat")
	xlsx.SetCellValue(rekapSheet, "G4", "Total")
	xlsx.SetCellStyle(rekapSheet, "A4", "G4", headerStyle)
	xlsx.SetColWidth(rekapSheet, "A", "F", 17)
	xlsx.MergeCell(rekapSheet, "A1", "G1")
	xlsx.MergeCell(rekapSheet, "A2", "G2")
	xlsx.SetCellValue(rekapSheet, "A1", "Report Asuransi Periode "+tgl1+" - "+tgl2)
	xlsx.SetCellValue(rekapSheet, "A2", user.Name)
	xlsx.SetCellStyle(rekapSheet, "A1", "G1", headerStyle)
	xlsx.SetCellStyle(rekapSheet, "A2", "G2", headerStyle)

	startMergeRekap := startRow
	awalBorder := startRow
	for index, value := range rekapSourceInfo {
		currentSource := "Excell"
		if user.DataSource == "W" {
			currentSource = "Wanda"
		}
		if index+1 == len(rekapSourceInfo) {
			xlsx.MergeCell(rekapSheet, fmt.Sprintf("A%d", startMergeRekap), fmt.Sprintf("A%d", startRow))
		...
		xlsx.SetCellValue(rekapSheet, fmt.Sprintf("A%d", startRow), value["kd_user"])
		xlsx.SetCellValue(rekapSheet, fmt.Sprintf("B%d", startRow), currentSource)
		xlsx.SetCellValue(rekapSheet, fmt.Sprintf("C%d", startRow), bulan[value["bulan"].(int64)])
		xlsx.SetCellValue(rekapSheet, fmt.Sprintf("D%d", startRow), value["p"])
		xlsx.SetCellValue(rekapSheet, fmt.Sprintf("E%d", startRow), value["t"])
		xlsx.SetCellValue(rekapSheet, fmt.Sprintf("F%d", startRow), value["o"])
	...
	xlsx.SetCellStyle(rekapSheet, fmt.Sprintf("A%d", awalBorder), fmt.Sprintf("G%d", startRow-1), borderStyle)

	startRow += 1
	xlsx.SetCellValue(rekapSheet, fmt.Sprintf("A%d", startRow), "Nomor Mesin")
	xlsx.SetCellValue(rekapSheet, fmt.Sprintf("B%d", startRow), "Nama Customer")
	xlsx.SetCellValue(rekapSheet, fmt.Sprintf("C%d", startRow), "Nama Dealer")
	xlsx.SetCellValue(rekapSheet, fmt.Sprintf("D%d", startRow), "Status")
	xlsx.SetCellValue(rekapSheet, fmt.Sprintf("E%d", startRow), "Tanggal Verifikasi")
	xlsx.SetCellStyle(rekapSheet, fmt.Sprintf("A%d", startRow), fmt.Sprintf("E%d", startRow), headerStyle)

	startRow += 1
	startMergeRekap = startRow
	awalBorder = startRow
	for _, value := range rekapKdUser {
		stsAsuransi := ""
		if value["sts_asuransi"] == "P" {
			stsAsuransi = "Pending"
		} else if value["sts_asuransi"] == "O" {
			stsAsuransi = "Berminat"
		} else if value["sts_asuransi"] == "T" {
			stsAsuransi = "Tidak Berminat"
		}
		xlsx.SetCellValue(rekapSheet, fmt.Sprintf("A%d", startRow), value["no_msn"])
		xlsx.SetCellValue(rekapSheet, fmt.Sprintf("B%d", startRow), value["nm_customer11"])
		xlsx.SetCellValue(rekapSheet, fmt.Sprintf("C%d", startRow), value["nm_dlr"])
		xlsx.SetCellValue(rekapSheet, fmt.Sprintf("D%d", startRow), stsAsuransi)
		xlsx.SetCellValue(rekapSheet, fmt.Sprintf("E%d", startRow), value["tgl_verifikasi"].(time.Time).Format("2006-01-02"))
		startRow += 1
	}
	xlsx.SetCellStyle(rekapSheet, fmt.Sprintf("A%d", awalBorder), fmt.Sprintf("E%d", startRow-1), borderStyle)
